...
def main() -> None:
    ...
    # 14 days attack, varying probes/day
    # -----------------------------------------------------------------------
    ...
    for probes_per_day in probe_rates:
        for label, builder in (
            ...
            cfg = builder(n_makers, fee_sats, seed=1000)
            ...
            deanon = snap_last_attack[-1]["taker_deanonymized_fraction"] if snap_last_attack else 0
    ...
    for fee in fee_levels:
        for label, builder in (
            ...
            cfg = builder(n_makers, fee, seed=2000)
            attack_cfg = SustainedAttackConfig(
                n_days=30,
                honest_cjs_per_day=honest_cjs_per_day,
                probes_per_day=probes_per_day,
    ...
    for probes_per_day in recovery_probes:
        for label, builder in (
            ...
            cfg = builder(n_makers, 500, seed=3000)
            attack_cfg = SustainedAttackConfig(
                n_days=44,
                honest_cjs_per_day=honest_cjs_per_day,
                probes_per_day=probes_per_day,
    ...
    for n_mk in maker_counts:
        for label, builder in (
            ...
            cfg = builder(n_mk, 500, seed=5000)
            attack_cfg = SustainedAttackConfig(
                n_days=30,
                honest_cjs_per_day=honest_cjs_per_day,
                probes_per_day=10,
    ...
    for probes_per_day in high_probe_rates:
        for label, builder in (
            ...
            cfg = builder(n_makers, 500, seed=6000)
            attack_cfg = SustainedAttackConfig(
                n_days=30,
                honest_cjs_per_day=honest_cjs_per_day,
                probes_per_day=probes_per_day,
