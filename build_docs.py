...
def _load_kpis() -> dict[str, Any]:
    """Pull live numbers from the v7.3 reports (best-effort).

    Headline framing: maker-wallet clustering + taker anonymity-set
    reduction under the v7.3 chain-edge clusterer (change-chain plus
    fee-fingerprint equal-chain plus conservative non-CJ CIOH plus
    fidelity-bond funding-tx CIOH, precision = 1.0 by construction,
    ...
    # Probe-attack validation (v7.3 ground truth)
    probe_p = TMP / "v7" / "probe_validation_v73.json"
    if probe_p.exists():
        p = json.loads(probe_p.read_text())
        kpis["probe_nicks_total"] = p.get("n_nicks")
        kpis["probe_nicks_matched"] = p.get("n_nicks_with_any_v73_match")
...
({n_txs:,} JM CoinJoins, {n_decoded:,} ILP-decoded), the v7.3
clusterer (change-chain + fee-fingerprint equal-chain + non-CJ
CIOH + fidelity-bond funding-tx CIOH) recovers {n_clusters:,} certified wallet components. Each
certified maker shrinks the taker's per-CJ anonymity set.</p>
<div class="kpis">
  <div class="kpi"><b>{mean_n_eq:.2f} &rarr; {mean_residual:.2f}</b><span>Mean taker anonymity set (published &rarr; v7.3 residual)</span></div>
  <div class="kpi danger"><b>{share_any:.1f}%</b><span>CJs where at least one maker is certified</span></div>
  <div class="kpi"><b>{n_clusters:,}</b><span>v7.3 maker clusters ({n_nontrivial:,} non-trivial)</span></div>
...
<p><em>May 2026 (coinjoin-simulator + joinmarket-analyzer, v7.3 clusterer)</em></p>
<div class="kpis">
  ...
  <div class="kpi"><b>{n_clusters:,}</b><span>v7.3 clusters ({n_nontrivial:,} non-trivial, largest {largest})</span></div>
  ...
  <div class="kpi safe"><b>{probe_collisions} / {probe_matched}</b><span>Cross-nick collisions / probed nicks with v7.3 matches (of {probe_total} probed)</span></div>
</div>
"""
