"""Publication figures for the v7.2 clusterer.

Outputs to ``papers/figures/``. Reuses the v7 builder shape and
overwrites figures whose data has shifted under v7.1 / v7.2:

  cluster_size_distribution.svg  v7.2 cluster size histogram
  anonset_reduction_hist.svg     v7.2 residual anonymity-set
  anonset_per_n_eq.svg           v7.2 per-n_eq breakdown
  v6_vs_v7_anonset_overlay.svg   v6 / v7 / v7.1 / v7.2 overlay
  probe_validation_v6.svg        same name, v7.2 numbers (cards)

The v7-only attribution breakdown and the v5/v6 fragmentation
figure are unchanged in structure (v7.1 / v7.2 only add non-CJ
CIOH edges; the v7 attribution table is untouched).
"""
...
PROBE = TMP / "v7" / "probe_validation_v72.json"
V72_REPORT = TMP / "v7" / "mainnet_v72_report.json"
...
def anonset_reduction_hist_v7() -> None:
    d = json.loads(ANON_V72.read_text())
    ...
    ax.set_title(
        "Per-CJ residual anonymity-set after removing v7.2-certified makers",
    )
    ax.set_xticks(xs)
    for x, y in items:
        pct = 100 * y / total
...
def anonset_per_n_eq_v7() -> None:
    ...
    ax.bar(
        ...
        color=COL["red"],
        edgecolor=COL["dark"],
        label="residual after removing v7.2-certified makers",
    )
    ax.set_xticks(x_pos)
...
def v6_vs_v7_anonset_overlay() -> None:
    ...
    for i, (name, h, c, d) in enumerate(hists):
        ...
        offset = (i - (n - 1) / 2) * w
        ax.bar(
            [p + offset for p in x_pos], ys, width=w,
            color=c, edgecolor=COL["dark"], linewidth=0.3,
    ...
    ax.set_xlabel("residual anonymity-set size (lower bound)")
    ax.set_ylabel("number of coinjoins")
    ax.set_title("Residual anonset by clusterer iteration (v6 -> v7.2)")
    ax.legend(loc="upper right", fontsize=9, framealpha=0.95)
    _save(fig, "v6_vs_v7_anonset_overlay.svg")


def cluster_size_distribution_v7() -> None:
    report = json.loads(V72_REPORT.read_text())
    ...
    ax.set_ylabel("number of v7.2 clusters")
    n_total = report["n_clusters"]
    ax.set_title(f"v7.2 maker-cluster size distribution (mainnet, n={n_total:,} clusters)")
    ax.set_yscale("log")
    if max(xs) > 20:
        ax.set_xscale("log")
...
def probe_validation_v7() -> None:
    ...
    cards = [
        ...
        ("nicks matched", f"{d['n_nicks_with_any_v72_match']}"),
        ("UTXOs matched", f"{d['total_matched_in_v72']} / {d['total_offered_utxos']}"),
    ...
    for i, (label, value) in enumerate(cards):
        ...
        ax.text(x, 0.3, label, ha="center", va="center", fontsize=10,
                color=COL["dark"], transform=ax.transAxes)
    ax.set_title(
        "v7.2 probing-attack ground-truth validation: 0 cross-nick collisions",
        pad=12,
    )
