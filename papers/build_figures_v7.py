"""Publication figures for the v7 fee-fingerprint clusterer.

Outputs to ``papers/figures/``. Overwrites the v6 figures where
their meaning has changed (anonset histograms, per-n_eq table,
cluster-size distribution, probe validation). Adds two new
figures:

  v6_vs_v7_anonset_overlay.svg   anonset reduction histogram, v6
                                  (n_eq-corrected) baseline overlaid
                                  with v7

  v7_attribution_breakdown.svg   how the v7 fee-fingerprint step
                                  classifies the 51,439 cross-CJ
                                  equal-output reuses
"""
...
PROBE = TMP / "v7" / "probe_validation_v7.json"
V7_REPORT = TMP / "v7" / "mainnet_v7_report.json"
...
def anonset_reduction_hist_v7() -> None:
    d = json.loads(ANON_V7.read_text())
    ...
    ax.set_title(
        "Per-CJ residual anonymity-set after removing v7-certified makers",
    )
    ax.set_xticks(xs)
    for x, y in items:
        pct = 100 * y / total
...
def anonset_per_n_eq_v7() -> None:
    ...
    ax.bar(
        ...
        color=COL["red"],
        edgecolor=COL["dark"],
        label="residual after removing v7-certified makers",
    )
    ax.set_xticks(x_pos)
...
def v6_vs_v7_anonset_overlay() -> None:
    ...
    h6 = {int(k): int(v) for k, v in d6["residual_anon_set_histogram"].items()}
    h7 = {int(k): int(v) for k, v in d7["residual_anon_set_histogram"].items()}
    xs = sorted(set(h6) | set(h7))
    ys6 = [h6.get(x, 0) for x in xs]
    ys7 = [h7.get(x, 0) for x in xs]
    fig, ax = plt.subplots(figsize=(7.6, 3.8))
    w = 0.4
    x_pos = list(range(len(xs)))
    ax.bar(
        [p - w / 2 for p in x_pos], ys6, width=w,
        color=COL["grey"], edgecolor=COL["dark"],
        label=f"v6 (mean residual {d6['mean_residual_anon_set']:.2f})",
    )
    ax.bar(
        [p + w / 2 for p in x_pos], ys7, width=w,
        color=COL["blue"], edgecolor=COL["dark"],
        label=f"v7 (mean residual {d7['mean_residual_anon_set']:.2f})",
    )
    ax.set_xticks(x_pos)
    ...
    ax.set_ylabel("number of coinjoins")
    ax.set_title(
        "v6 (constrained UF) vs v7 (with fee-fingerprint): residual anonset",
    )
...
def cluster_size_distribution_v7() -> None:
    report = json.loads(V7_REPORT.read_text())
    ...
    ax.set_ylabel("number of v7 clusters")
    n_total = report["n_clusters"]
    ax.set_title(f"v7 maker-cluster size distribution (mainnet, n={n_total:,} clusters)")
    ax.set_yscale("log")
    if max(xs) > 20:
        ax.set_xscale("log")
...
def probe_validation_v7() -> None:
    ...
    cards = [
        ...
        ("nicks matched", f"{d['n_nicks_with_any_v7_match']}"),
        ("UTXOs matched", f"{d['total_matched_in_v7']} / {d['total_offered_utxos']}"),
    ...
    for i, (label, value) in enumerate(cards):
        ...
        ax.text(x, 0.3, label, ha="center", va="center", fontsize=10,
                color=COL["dark"], transform=ax.transAxes)
    ax.set_title(
        "v7 probing-attack ground-truth validation: 0 cross-nick collisions",
        pad=12,
    )
